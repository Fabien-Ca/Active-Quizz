    ...
    activequiz.ajax.create_request('/mod/activequiz/quizinfo.php', params, function (status, response) {
        ...
        if (status == 500) {
            ...
            if (response.status == 'notrunning') {
                ...
                // gather the current results and stats
                if (activequiz.get('delayrefreshresults') === 'undefined' || activequiz.get('delayrefreshresults') === 'false') {
                    ...
                    /****************** modif PRT 2406 ******************/
                    activequiz.gather_current_stats();
                    /**************** Fin modif PRT 2406 ****************/
               }
...
activequiz.start_quiz = function () {
    ...
    this.control_buttons(['endquestion', 'toggleresponses', 'togglenotresponded', 'togglestats']);
};
...
activequiz.gather_current_results = function () {
    ...
    // ici
    // console.log("getcurrentresults");
    // console.log(params);
    // https://401patrick.devlmsfactory.com//mod/activequiz/quizdata.php?sesskey=ucrqn6e2NX&action=getcurrentresults&attemptid=81&rtqid=6&sessionid=42
...
/****************** modif PRT 2406 ******************/
activequiz.gather_current_stats = function () {
    if (activequiz.get('showstudentstats') === false) {
        return; // return if there we aren't showing student responses
    }

    var params = {
        'action': 'getcurrentstats',
        'rtqid': activequiz.get('rtqid'),
        'sessionid': activequiz.get('sessionid'),
        'attemptid': activequiz.get('attemptid'),
        'sesskey': activequiz.get('sesskey')
    ...
    // console.log(params);
    // https://401patrick.devlmsfactory.com//mod/activequiz/quizdata.php?sesskey=rUUCRUFo8T&action=getcurrentstats&attemptid=89&rtqid=6&sessionid=46

    activequiz.ajax.create_request('/mod/activequiz/quizdata.php', params, function (status, response) {

        if (status == '500') {
            activequiz.quiz_stats('there was an error getting current results', true);
        } else if (status == 200) {
            activequiz.quiz_stats(response.responses, true);

            // after the responses have been inserted, we see if any question type javascript was added and evaluate
            if (document.getElementById('stats_js') !== null) {
                eval(document.getElementById('stats_js').innerHTML);
            }
        }
        ...
        if (activequiz.get('endquestion') == 'true') {
            ...
            if (activequiz.get('lastquestion') != 'undefined') {
                ...
                if (activequiz.get('lastquestion') == 'true') { // don't enable the next question button
                    ...
                    activequiz.control_buttons(['closesession', 'reloadresults', 'jumptoquestion', 'repollquestion', 'showcorrectanswer', 'toggleresponses', 'togglestats']);
                } else {
                    //otherwise enable the next question button and repoll question

                    activequiz.control_buttons(['closesession', 'nextquestion', 'jumptoquestion', 'repollquestion', 'reloadresults', 'showcorrectanswer', 'toggleresponses', 'togglestats']);
                }
            } else {
                activequiz.control_buttons(['closesession', 'nextquestion', 'jumptoquestion', 'repollquestion', 'reloadresults', 'showcorrectanswer', 'toggleresponses', 'togglestats']);
            }
...
activequiz.repoll_question = function () {
    ...
    activequiz.ajax.create_request('/mod/activequiz/quizdata.php', params, function (status, response) {
        ...
        activequiz.control_buttons(['endquestion', 'toggleresponses', 'togglenotresponded', 'togglestats']);
        activequiz.waitfor_question(response.questionid, response.questiontime, response.delay, response.nextstarttime);
    });
...
activequiz.next_question = function () {
    ...
    /****************** modif PRT 2406 ******************/
    activequiz.hide_responses();
    activequiz.hide_stats();
    /**************** Fin modif PRT 2406 ****************/
    this.control_buttons([]);
...
activequiz.close_session = function () {
    ...
    activequiz.ajax.create_request('/mod/activequiz/quizdata.php', params, function (status, response) {
        ...
        activequiz.clear_and_hide_qstatsbox();
...
/****************** modif PRT 2406 ******************/
activequiz.hide_responses = function () {
...
/****************** modif PRT 2406 ******************/
/**
 * Toggles the "show stats" variable
 */
activequiz.toggle_stats = function () {

    var togglestatsBtn = document.getElementById('togglestats');

    if (activequiz.get('showstudentstats') === false) { // if it is false, set it back to true for the student responses to show

        togglestatsBtn.innerHTML = M.util.get_string('hide_stats', 'activequiz');

        activequiz.set('showstudentstats', true);
        activequiz.gather_current_stats();
    } else { // if it's set to true, or not set at all, then set it to false when this button is clicked

        togglestatsBtn.innerHTML = M.util.get_string('show_stats', 'activequiz');
        activequiz.set('showstudentstats', false);
        activequiz.clear_and_hide_qstatsbox();
    }
};

activequiz.hide_stats = function () {
