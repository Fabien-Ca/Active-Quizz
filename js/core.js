...
activequiz.quiz_page_loaded = function (stop) {
    ...
    if (stop) {
        ...
        /****************** Modif PRT 2406 ******************/
        // hide the student responses by default
        if(activequiz.get('isinstructor')){
            activequiz.toggle_responses();
            activequiz.toggle_stats();
        }
        /**************** Fin modif PRT 2406 ****************/
...
/****************** modif PRT 2406 ******************/
/**
 * Util function to clear and hide the quizstatsbox
 *
 */
activequiz.clear_and_hide_qstatsbox = function () {

    var quizstatsbox = document.getElementById('quizstatsbox');

    if (!quizstatsbox.classList.contains('hidden')) {
        quizstatsbox.classList.add('hidden');
    }

    quizstatsbox.innerHTML = '';
};
/**************** Fin modif PRT 2406 ****************/
...
/****************** modif PRT 2406 ******************/
/**
 * Utility class to display information in the quizstatsbox
 *
 * @param quizinfo
 * @param clear  bool for whether or not to clear the quizstatsbox
 */
activequiz.quiz_stats = function (quizstats, clear) {
    ...
    if (typeof quizstats == 'object') {
        quizstatsbox.appendChild(quizstats);
    } else {
        quizstatsbox.innerHTML = quizstats;
    }

    // if it's hidden remove the hidden class
    if (quizstatsbox.classList.contains('hidden')) {
        quizstatsbox.classList.remove('hidden');
    }
};
/**************** Fin modif PRT 2406 ****************/
