...
class activequiz_session {
    ...
    /****************** modif PRT 2406 ******************/
    /**
     * Gets the stats of the current question
     *
     */
    public function get_question_stats(){
        // Next load all active attempts.
        $attempts = $this->getall_open_attempts(false);
        $slot = $this->session->currentqnum;
        ...
        foreach ($attempts as $attempt) {
            if($attempt->responded == 1){
                // ici voir le détail de summarize_response, il y a peut être trop par rapport au besoin
                $attempt->summarize_response($this->session->currentquestion);
                $quba = $attempt->get_quba();
                $qa = $quba->get_question_attempt($slot);
                $response = $qa->get_last_step()->get_qt_data();
                $question = $qa->get_question(false);

                $fraction = $question->grade_response($response)[0];
                ...
                $totalstatssummary .= $question->grade_response($response)[0]."<br>";
            }
        }
        $countdatasetdata = array($slice1, $slice2, $slice3, $slice4);

        // If there is no grade yet, nothing to show.
        if(max($countdatasetdata) == 0){
            return '';
        }
        ...
        $chartheight = 300;
        $chartwidth = 600;
        $labels = array("0 - 0,25", "0,25 - 0,5", "0,5 - 0,75", "0,75 - 1");

        $chartoutput = '';
        $chartoutput .= \html_writer::start_tag('div', array('id' => 'statsholder', 'style' => 'width:80%;height:300px'));
        $chartoutput .= \html_writer::tag('canvas', '', array('id' => 'statschart', 'width' => $chartwidth, 'height' => $chartheight));
        $chartoutput .= \html_writer::start_tag('script', array('type' => 'text/javascript', 'id' => 'stats_js'));
        ...
        if(max($countdatasetdata) >= 50){
            $scaleStepWidth = 10;
        }
        else if(max($countdatasetdata) >= 20){
            $scaleStepWidth = 5;
        }
        else if(max($countdatasetdata) >= 10){
            $scaleStepWidth = 2;
        }
        else{
            $scaleStepWidth = 1;
        ...
        $options->scaleSteps = ceil(max($countdatasetdata)/$scaleStepWidth);
    ...
    */
    ...
    public function getall_attempts($includepreviews, $open = 'all', $userid = null, $skipgroups = false, $orderinfo = null) {
    /**************** Fin modif PRT 2406 ****************/
        global $DB;
        ...
        if(!is_null($orderinfo) && !is_null($orderinfo["sortby"])){
            $sortby = $orderinfo["sortby"];
            // $sortorder = $orderinfo["sortorder"];
            if ($orderinfo["sortorder"] != SORT_ASC) {
                $sortorder = "DESC";
            } else {
                $sortorder = "ASC";
            } 
        ...
        $sql = "SELECT aa.id, aa.sessionid, aa.userid, aa.attemptnum, aa.questionengid, aa.status, aa.preview, aa.responded, aa.responded_count, aa.forgroupid, aa.timestart, aa.timefinish, aa.timemodified, aa.qubalayout, u.firstname AS firstname, u.lastname AS lastname FROM {activequiz_attempts} aa
        JOIN {user} u ON aa.userid = u.id
        WHERE $wherestring $sortby";
        /**************** Fin modif PRT 2406 ****************/
        ...
        foreach ($dbattempts as $dbattempt) {
            ...
            // All attempts except that of the instructor.
            $is_instructor = $this->rtq->has_capability('mod/activequiz:control', $dbattempt->userid);
            if(!$is_instructor){
                $attempts[ $dbattempt->id ] = new activequiz_attempt($this->rtq->get_questionmanager(), $dbattempt,
                    $this->rtq->getContext());
            }
