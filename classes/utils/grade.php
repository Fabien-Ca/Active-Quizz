...
class grade {
     ...
     * @return array
     */
    public static function get_user_grade($activequiz, $userid = 0, $orderinfo = null) {
        global $DB;
        ...
        return $DB->get_records_sql("
            ...
            $usertest
            GROUP BY u.id, rtqg.grade, rtqg.timemodified $sortby", $params);
    ...
    protected function process_sessions($sessions, $userid = null) {
        ...
        foreach ($sessions as $session) {
            ...
            // ici grade.php -> process_sessions
            // echo("process_sessions\n");
            // foreach ($session->get_session_users() as $user) {
            //     $uid = $user->userid;
            //     echo("get_session_grade : $uid\n");
            //     var_dump($this->get_session_grade($session, $uid));
            //     echo("\n");
            // }
            // die;
    ...
    protected function get_session_grade($session, $userid) {
        ...
        // ici grade.php -> get_session_grade
        $attempts = $session->getall_attempts(false, 'closed', $userid, true);
        // $attempts = $session->getall_attempts(false, 'all', $userid, true);
    ...
    public function calculate_attempt_grade($attempt) {
        ...
        foreach ($attempt->getSlots() as $slot) {
            // ici grade.php -> calculate_attempt_grade
            // get_question_mark repond 0 -> apriori pas de set_fraction
            // echo("get_question_max_mark : ");
            // var_dump($quba->get_question_max_mark($slot));
            // echo("get_question_fraction : ");
            // var_dump($quba->get_question_fraction($slot));
            // echo("get_question_mark : ");
            // var_dump($quba->get_question_mark($slot));
            // echo("\n");
