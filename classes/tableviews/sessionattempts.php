...
class sessionattempts extends \flexible_table implements \renderable {
    ...
    public function __construct($uniqueid, $rtq, $session, $pageurl) {
        ...
        $this->column_nosort[] = 'edit';
        // $this->column_nosort[] = 'attemptgrade';
    ...
    public function setup() {
        ...
        $columns = array(
            ...
            'attemptnum'   => get_string('attemptno', 'activequiz'),
            ...
            'grade'        => get_string('attempt_grade', 'activequiz'),
        );
    ...
    protected function get_data() {
        ...
        /****************** Modif PRT 2406 ******************/
        // Update current session grades for ordering by grade
        foreach ($attempts as $key => $attempt) {
            $grade = number_format($this->rtq->get_grader()->calculate_attempt_grade($attempt), 2);
            $attemptdata = new \stdClass();
            $attemptdata->id = $key;
            $attemptdata->grade = floatval($grade);
            $DB->update_record('activequiz_attempts', $attemptdata);
        }

        $attempts = $this->session->getall_attempts(true, 'all', null, false, $this->get_primary_sort_order());
        /**************** Fin modif PRT 2406 ****************/
        $userids = array();
